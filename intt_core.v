...
module intt_core #(
    ...
    localparam S_BR_RUN         = 4'd1;
    localparam S_BR_FLUSH       = 4'd2;
    localparam S_INTT_RUN       = 4'd3;
    localparam S_INTT_FLUSH     = 4'd4; 
    localparam S_POST_RUN       = 4'd5;
    localparam S_POST_FLUSH     = 4'd6; 
    ...
    // 【修改点】：深度+1，适应6拍乘法器
    reg [7:0] post_sr_val;         
    reg [7:0] post_sr_addr [0:7];  
    reg [7:0] post_rom_addr_reg;   
    ...
    wire [WIDTH-1:0] post_rom_data, intt_twiddle_data, mont_res, mod_add_out, mod_sub_out;
    post_rom u_post_rom (.clka(clk), .addra(post_rom_addr_reg), .douta(post_rom_data));
    intt_twiddle_rom u_intt_twiddle_rom (.clka(clk), .addra(rom_addr_reg), .douta(intt_twiddle_data));

    wire current_pp = sr_ping_pong[0];
    wire [WIDTH-1:0] current_u = (current_pp == 0) ? ram1_rdata_a : ram0_rdata_a;
    wire [WIDTH-1:0] current_v = (current_pp == 0) ? ram1_rdata_b : ram0_rdata_b;

    wire [WIDTH-1:0] mont_a_in = (state == S_POST_RUN || state == S_POST_FLUSH) ? ram1_rdata_a : current_v;
    wire [WIDTH-1:0] mont_b_in = (state == S_POST_RUN || state == S_POST_FLUSH) ? post_rom_data : intt_twiddle_data;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            sr_val <= 0; post_sr_val <= 0; sr_ping_pong <= 0;
            rom_addr_reg <= 0; post_rom_addr_reg <= 0; 
            
            br_sr_addr[0] <= 0; br_sr_addr[1] <= 0; br_sr_val <= 0;
            
            u_d1 <= 0; u_d2 <= 0; u_d3 <= 0; u_d4 <= 0; u_d5 <= 0; u_d6 <= 0;
            res_u_reg <= 0; res_v_reg <= 0;
            
            for(i=0; i<8; i=i+1) post_sr_addr[i] <= 0;
            ...
            if (state == S_INTT_RUN || state == S_INTT_FLUSH) begin
                sr_val <= {sr_val[7:0], pipe_en};
            ...
            if (state == S_POST_RUN || state == S_POST_FLUSH) begin
                post_sr_val <= {post_sr_val[6:0], pipe_en};
                post_sr_addr[0] <= cnt[7:0];
                for (i=1; i<8; i=i+1) post_sr_addr[i] <= post_sr_addr[i-1];
            end
            ...
            case (state)
                S_IDLE: begin
                    ...
                    if (start) begin 
                        cnt <= 0; pipe_en <= 1; state <= S_BR_RUN; 
                        stage_cnt <= 0; bf_cnt <= 0;
                        sr_val <= 0; post_sr_val <= 0; br_sr_val <= 0; sr_ping_pong <= 0;
                        u_d1 <= 0; u_d2 <= 0; u_d3 <= 0; u_d4 <= 0; u_d5 <= 0; u_d6 <= 0;
                        res_u_reg <= 0; res_v_reg <= 0;
                    end
                end
                ...
                S_BR_RUN: begin
                    ...
                    if (br_sr_val[1]) begin
                        ram1_we_a <= 1; ram1_addr_a <= br_sr_addr[1]; ram1_wdata_a <= ram0_rdata_a;
                    end
                end
                S_BR_FLUSH: begin
                    if (br_sr_val[1]) begin
                    ...
                    if (br_sr_val == 0) begin
                        stage_cnt <= 0; bf_cnt <= 0; pipe_en <= 1; sr_val <= 0; state <= S_INTT_RUN;
                    end
                end
                ...
                S_INTT_RUN: begin
                    if (pipe_en) begin
                        ...
                        if (bf_cnt == 127) begin
                            bf_cnt <= 0; pipe_en <= 0; state <= S_INTT_FLUSH;
                ...
                S_INTT_FLUSH: begin
                    ...
                    if (sr_val == 9'b0) begin 
                        if (stage_cnt == 7) begin
                            cnt <= 0; pipe_en <= 1; state <= S_POST_RUN; 
                        end else begin
                            stage_cnt <= stage_cnt + 1; pipe_en <= 1; state <= S_INTT_RUN;
                        end
                    end
                end
                ...
                S_POST_RUN: begin
                    if (pipe_en) begin
                        ram1_addr_a <= cnt[7:0]; post_rom_addr_reg <= cnt[7:0];
                        if (cnt == 255) begin pipe_en <= 0; state <= S_POST_FLUSH; end 
                        else cnt <= cnt + 1;
                    end
                    if (post_sr_val[7]) begin
                        ram0_we_a <= 1;
                        ram0_addr_a <= post_sr_addr[7]; ram0_wdata_a <= mont_res; 
                    end
                end
                S_POST_FLUSH: begin
                    if (post_sr_val[7]) begin
